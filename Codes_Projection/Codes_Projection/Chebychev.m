% Stochastic neoclassical growth model with CRRA utility using Chebychev polynomials
%
% This file calls:
%
...
% (iv) simulation.m
%
% Dario Caldara and Jesus Fernandez-Villaverde
%
% Princeton, 4/7/2014
...
shock_num = 5;                     % number of nodes for technology process Z
nodeSizeList = [20,50];             % number of polynomials for capital
...
vss = ((css^nu*(1-lss)^(1-nu))^(1-gamma))/(1-gamma);
...
for multinode_step = 1:listSize
    ...
    for i1 = 3:node_num
        ...
        rho = residual_fcn(alpha,beta,delta,nu,gamma,k_min,k_max,rho_guess,grid_k,T_k,Z,PI,node_num,shock_num,M);
        rho_old = rho;
...
[g_k,g_c,g_l,value_fcn,euler_error,max_error]= ...
    eulerr_grid(alpha,beta,delta,gamma,nu,rho,Z,PI,...
...
[vSeries,kSeries,cSeries,lSeries,ySeries,...
    ...
    simulation(alpha,beta,delta,gamma,nu,kss,rho,Z,PI,k_min,k_max,node_num,shock_num,M,T,dropT);
